...
function normalizeRawCredentialEntry(
  ...
  const providerForAliases = typeof entry.provider === "string" ? entry.provider : fallbackProvider;
  if (entry.type === "oauth" && normalizeProviderId(providerForAliases ?? "") === "openai-codex") {
...
function parseCredentialEntry(
  ...
  const typed = normalizeRawCredentialEntry(raw, fallbackProvider);
