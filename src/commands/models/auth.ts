...
export async function modelsAuthPasteTokenCommand(
  ...
  const tokenValidationMessage = (value: string | undefined): string | undefined => {
    const trimmed = value?.trim();
    if (!trimmed) {
      return "Required";
    }
  ...
  const directTokenValidationError = rawToken ? tokenValidationMessage(rawToken) : undefined;
  if (directTokenValidationError) {
    throw new Error(directTokenValidationError);
  }
  const tokenInput =
    rawToken ??
    (await readPastedSecret({
      ...
      masked: false,
      validate: tokenValidationMessage,
    }));
  const token =
    provider === "anthropic"
